...
class UsuarioFunctions {
    ...
    public create() {
        return [check(['nombre', 'img']).trim().isLength({ min: 3 }).withMessage('Error En El Atributo Nombre | Valor'),
        ...
        check('role').trim().isLength({ min: 3 }).isIn(this.roles).withMessage('Error En El Atributo Role | Valor'), usuariosController.create];
    }
