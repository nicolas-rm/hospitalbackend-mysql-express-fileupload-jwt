...
class UsuarioFunctions {
    ...
    create() {
        return [express_validator_1.check(['nombre', 'img']).trim().isLength({ min: 3 }).withMessage('Error En El Atributo Nombre | Valor'),
            ...
            express_validator_1.check('role').trim().isLength({ min: 3 }).isIn(this.roles).withMessage('Error En El Atributo Role | Valor'), usuarios_controller_1.default.create];
    }
    read() {
...
const usuarioFunctions = new UsuarioFunctions();
exports.default = usuarioFunctions;
